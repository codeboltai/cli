...
const readConfig = async (folderPath) => {
    const configPath = path.join(folderPath, 'codeboltactionblock.yaml');
    if (!fs.existsSync(configPath)) {
        throw new Error('codeboltactionblock.yaml not found. Please run this command in an actionblock directory.');
    }
    const config = yaml.load(fs.readFileSync(configPath, 'utf8'));
    if (!config.name) throw new Error('name is required in codeboltactionblock.yaml');
    if (!config.unique_id) throw new Error('unique_id is required in codeboltactionblock.yaml');
    if (!config.description) throw new Error('description is required in codeboltactionblock.yaml');
    if (!config.version) throw new Error('version is required in codeboltactionblock.yaml');
    if (!config.author) throw new Error('author is required in codeboltactionblock.yaml');
    return config;
};

const publishActionBlock = async (targetPath) => {
    if (!checkUserAuth()) {
    ...
    try {
        ...
        console.log(chalk.blue('Processing the ActionBlock....'));
        const folderPath = targetPath || '.';
        const folder = path.resolve(folderPath);
        const config = await readConfig(folderPath);
        console.log(chalk.green(`Found actionblock configuration: ${config.name}`));
        ...
        const distResult = await uploadFile(distZipPath, 'actionblock', authToken);
        console.log(chalk.blue('Uploading source code...'));
        const sourceResult = await uploadFile(sourceZipPath, 'actionblocksource', authToken);
        ...
        const response = await axios.post('https://api.codebolt.ai/api/actionblocks/add', {
        ...
        console.log(chalk.blue(`ActionBlock ID: ${config.unique_id}`));
    } catch (error) {
...
module.exports = { publishActionBlock };
