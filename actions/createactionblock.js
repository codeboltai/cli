...
function createProject(projectName, installPath, answers) {
    ...
    const templatePath = path.resolve(__dirname, '..', 'template', 'actionblock');
    ...
    const yamlPath = path.join(projectDir, 'codeboltactionblock.yaml');
    ...
    console.log('Success! Your new action block is ready.');
...
const createactionblock = async (options) => {
    ...
    const templateYaml = yaml.load(fs.readFileSync(path.resolve(__dirname, '..', 'template/actionblock', 'codeboltactionblock.yaml'), 'utf8'));

    let answers;
    if (!quickEnabled) {
        answers = await inquirer.prompt([
            { type: 'input', name: 'projectName', message: 'ActionBlock name:', default: projectName },
        ...
        answers = {
            ...
            description: templateYaml.description || '', entryPoint: templateYaml.entryPoint || 'index.js',
