...
function createProject(projectName, installPath, answers) {
    ...
    const templatePath = path.resolve(__dirname, '..', 'template', 'capability');
    ...
    const yamlPath = path.join(projectDir, 'codeboltcapability.yaml');
    ...
    yamlObj.type = answers.type || 'skill';
    ...
    console.log('Success! Your new capability is ready.');
    console.log(`Created ${projectName} at ${projectDir}`);
}

const createcapability = async (options) => {
    let projectName = options.name || process.argv[3];
    const quickEnabled = options.quick || false;

    const templateYaml = yaml.load(fs.readFileSync(path.resolve(__dirname, '..', 'template/capability', 'codeboltcapability.yaml'), 'utf8'));

    let answers;
    if (!quickEnabled) {
        answers = await inquirer.prompt([
            { type: 'input', name: 'projectName', message: 'Capability name:', default: projectName },
            ...
            { type: 'list', name: 'type', message: 'Capability type:', choices: ['skill', 'power', 'talent'], default: templateYaml.type || 'skill' },
        ...
        answers = {
            ...
            description: templateYaml.description || '', type: templateYaml.type || 'skill',
