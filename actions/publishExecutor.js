...
const readConfig = async (folderPath) => {
    const configPath = path.join(folderPath, 'codeboltexecutor.yaml');
    if (!fs.existsSync(configPath)) {
        throw new Error('codeboltexecutor.yaml not found. Please run this command in an executor directory.');
    }
    const config = yaml.load(fs.readFileSync(configPath, 'utf8'));
    if (!config.name) throw new Error('name is required in codeboltexecutor.yaml');
    if (!config.unique_id) throw new Error('unique_id is required in codeboltexecutor.yaml');
    if (!config.description) throw new Error('description is required in codeboltexecutor.yaml');
    if (!config.version) throw new Error('version is required in codeboltexecutor.yaml');
    if (!config.author) throw new Error('author is required in codeboltexecutor.yaml');
    return config;
};

const publishExecutor = async (targetPath) => {
    ...
    try {
        ...
        console.log(chalk.blue('Processing the Executor....'));
        const folderPath = targetPath || '.';
        const folder = path.resolve(folderPath);
        const config = await readConfig(folderPath);
        console.log(chalk.green(`Found executor configuration: ${config.name}`));
        ...
        const distResult = await uploadFile(distZipPath, 'executor', authToken);
        console.log(chalk.blue('Uploading source code...'));
        const sourceResult = await uploadFile(sourceZipPath, 'executorsource', authToken);
        ...
        const supportedTypes = config.supportedTypes ? (Array.isArray(config.supportedTypes) ? config.supportedTypes.join(',') : config.supportedTypes) : 'skill';

        const response = await axios.post('https://api.codebolt.ai/api/executors/add', {
            ...
            version: config.version, entryPoint: config.entryPoint || 'index.js',
            zipFilePath: distResult.publicUrl, sourceCodeUrl: sourceResult.publicUrl,
            createdByUser: username
        }, { headers: { 'Authorization': `Bearer ${authToken}` } });

        console.log(chalk.green(response.data.message));
        console.log(chalk.blue(`Executor ID: ${config.unique_id}`));
        console.log(chalk.blue(`Supported Types: ${supportedTypes}`));
    } catch (error) {
...
module.exports = { publishExecutor };
