...
const readConfig = async (folderPath) => {
    const configPath = path.join(folderPath, 'codeboltcapability.yaml');
    if (!fs.existsSync(configPath)) {
        throw new Error('codeboltcapability.yaml not found. Please run this command in a capability directory.');
    }
    const config = yaml.load(fs.readFileSync(configPath, 'utf8'));
    if (!config.name) throw new Error('name is required in codeboltcapability.yaml');
    if (!config.unique_id) throw new Error('unique_id is required in codeboltcapability.yaml');
    if (!config.description) throw new Error('description is required in codeboltcapability.yaml');
    if (!config.version) throw new Error('version is required in codeboltcapability.yaml');
    if (!config.author) throw new Error('author is required in codeboltcapability.yaml');
    return config;
};

const publishCapability = async (targetPath) => {
    ...
    try {
        ...
        console.log(chalk.blue('Processing the Capability....'));
        const folderPath = targetPath || '.';
        const folder = path.resolve(folderPath);
        const config = await readConfig(folderPath);
        console.log(chalk.green(`Found capability configuration: ${config.name}`));
        ...
        const distResult = await uploadFile(distZipPath, 'capability', authToken);
        console.log(chalk.blue('Uploading source code...'));
        const sourceResult = await uploadFile(sourceZipPath, 'capabilitysource', authToken);
        ...
        const response = await axios.post('https://api.codebolt.ai/api/capabilities/add', {
            name: config.name, unique_id: config.unique_id, description: config.description,
            type: config.type || 'skill', tags: config.tags ? config.tags.join(', ') : '',
        ...
        console.log(chalk.blue(`Capability ID: ${config.unique_id}`));
    } catch (error) {
...
module.exports = { publishCapability };
