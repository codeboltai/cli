...
const readConfig = async (folderPath) => {
    const configPath = path.join(folderPath, 'codeboltskill.yaml');
    if (!fs.existsSync(configPath)) {
        throw new Error('codeboltskill.yaml not found. Please run this command in a skill directory.');
    }
    const config = yaml.load(fs.readFileSync(configPath, 'utf8'));
    if (!config.name) throw new Error('name is required in codeboltskill.yaml');
    if (!config.unique_id) throw new Error('unique_id is required in codeboltskill.yaml');
    if (!config.description) throw new Error('description is required in codeboltskill.yaml');
    if (!config.version) throw new Error('version is required in codeboltskill.yaml');
    if (!config.author) throw new Error('author is required in codeboltskill.yaml');
    return config;
};
...
const publishSkill = async (targetPath) => {
    ...
    try {
        ...
        console.log(chalk.green(`Found skill configuration: ${config.name}`));
        ...
        const distResult = await uploadFile(distZipPath, 'skill', authToken);
        console.log(chalk.blue('Uploading source code...'));
        const sourceResult = await uploadFile(sourceZipPath, 'skillsource', authToken);
        ...
        const response = await axios.post('https://api.codebolt.ai/api/skills/add', {
        ...
        console.log(chalk.blue(`Skill ID: ${config.unique_id}`));
    } catch (error) {
