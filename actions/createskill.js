...
function createProject(projectName, installPath, answers) {
    ...
    const templatePath = path.resolve(__dirname, '..', 'template', 'skill');
    ...
    const yamlPath = path.join(projectDir, 'codeboltskill.yaml');
    ...
    console.log('Success! Your new skill is ready.');
...
const createskill = async (options) => {
    ...
    const templateYaml = yaml.load(fs.readFileSync(path.resolve(__dirname, '..', 'template/skill', 'codeboltskill.yaml'), 'utf8'));

    let answers;
    if (!quickEnabled) {
        answers = await inquirer.prompt([
            { type: 'input', name: 'projectName', message: 'Skill name:', default: projectName },
