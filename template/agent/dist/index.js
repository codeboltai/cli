...
const processor_pieces_1 = require("@codebolt/agent/processor-pieces");
const unified_2 = require("@codebolt/agent/unified");
let systemPrompt = `
...
codeboltjs_1.default.onMessage(async (reqMessage) => {
    try {
        let promptGenerator = new unified_1.InitialPromptGenerator({
            processors: [
                ...
                new processor_pieces_1.AtFileProcessorModifier({
        ...
        let completed = false;
        do {
            let agent = new unified_2.AgentStep({ preInferenceProcessors: [], postInferenceProcessors: [] });
            let result = await agent.executeStep(reqMessage, prompt);
            prompt = result.nextMessage;
            let responseExecutor = new unified_1.ResponseExecutor({
